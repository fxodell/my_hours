...
async def create_time_entry(
    ...
    if not entry_data.description or not entry_data.description.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Description is required",
        )

    # Require job_code_id when location has active job codes
    if entry_data.location_id:
        jc_count = (await db.execute(
            select(func.count()).select_from(JobCode).where(
                JobCode.location_id == entry_data.location_id,
                JobCode.company_id == current_user.company_id,
                JobCode.is_active == True,
            )
        )).scalar()
